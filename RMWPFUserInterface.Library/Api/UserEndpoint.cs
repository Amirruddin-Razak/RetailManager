    ...
    public class UserEndpoint : IUserEndpoint
    {
        ...
        public async Task AddUserToRole(string userId, string role)
        {
            using (HttpResponseMessage response =
                await _apiHelper.ApiClient.PostAsJsonAsync("/api/User/Admin/AddRole", new { userId, role }))
            {
        ...
        {
            using (HttpResponseMessage response =
                await _apiHelper.ApiClient.PostAsJsonAsync("/api/User/Admin/RemoveRole", new { userId, role }))
            {
                if (response.IsSuccessStatusCode == false)
                {
                    throw new Exception(response.ReasonPhrase);
